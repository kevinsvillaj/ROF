...
$sql = "SELECT * FROM art_artcl ORDER BY article_id DESC";
...
$sql = "SELECT * FROM art_artcl ORDER BY article_id DESC LIMIT {$start_index}, {$page_size}";
$result = mysqli_query($conn, $sql);
...
if ($num_pages > 1) {
    ...
    for ($i = 1; $i <= $num_pages; $i++) {
        if ($i == $current_page) {
            ...
            echo "<a href='art.php?page={$i}'>{$i}</a> ";
        }
    }
}
